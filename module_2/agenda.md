## 📋 Class Agenda - Wednesday, September 2nd, 2026 

- 👋 **Welcome** (5 mins)
- 🤝 **Network** (10 mins – 2 mins each)
  - 💙 What's your dream job? 
  - 🔤 Person with the first name closest to "Z" starts, sorted Z–A
- 🧹 **Housekeeping** (10 mins)
  - Tell teachers if not completing assignment on time (just like a job!) 16 completions / 26 students = 61%
  - Update your picture in Slack & GitHub! Use an AI headshot ;)
  - [Test your GitHub knowledge](https://claude.ai/public/artifacts/222f959d-ba8d-448d-bebc-58982f71ab82) (shoutout to Kelly!)
    - What is still confusing? What makes sense?
  - AI Show & Tell from Kelsey  
- 👥 **Module 2 Review** (20 mins)
  - 🎤 How did the video lessons go?
      - Excel vs Google sheets? 
  - 🖥️ Share your screen & let's review together - data cleaning, pivot tables, charts, conditional formatting etc. 
- ⏸️ **8:00 PM – Break** (5 mins)
- 💸 **Assignment Overview** OR breakout groups for folks that still need to finish Module 1. 
- 💻 **If time - Hands On Activity**
- ✅ **Wrap Up & Questions** (last 10 mins)

## 📋 Class Agenda - Saturday, September 5th, 2026 

- 👋 **Welcome**
- **[Spreadsheet Quiz](https://claude.ai/public/artifacts/8a89bd93-19a9-4eb8-b9a7-b650da2c6228)**
- 🧹 **Housekeeping**
   - Change of schedule
   - Using Projects in Claude, ChatGPT, and Gemini
   - Congrats, Ashleigh! 🎉
- **Assignment Review**
   - How did the assignment go?
   - How comfortable do you feel using spreadsheets to analyze data?
- **Analysis Concepts**
   - Growth rate, % of total, and other key metrics
   - How to think like an analyst
   - How can you automate your finances going forward?
- **Hands-On Activity**
   - Using AI to analyze spending data
-  **Look Ahead: Module 3**
-  **Bonus Assignments**
-  **Guest Speaker: Alyson La**
